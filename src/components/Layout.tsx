...
export default function Layout({ 
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row">
      ...
      {/* Desktop Sidebar */}
      <aside className={`hidden md:flex flex-col bg-slate-900 text-white min-h-screen border-r border-slate-800 flex-shrink-0 transition-all duration-300 ${
      ...
      {/* Mobile Header Bar */}
      <header className="md:hidden bg-slate-900 text-white p-4 flex items-center justify-between border-b border-slate-800 animate-fade-in">
        <div className="flex items-center gap-3">
          {school?.logo_url ? (
      ...
      {mobileMenuOpen && (
        <>
          ...
          />
          {/* Drawer Content */}
          <aside className="fixed inset-y-0 left-0 w-64 bg-slate-900 text-white z-50 p-6 flex flex-col md:hidden animate-fade-in-left shadow-2xl border-r border-slate-800">
            {/* Drawer Header */}
            <div className="flex items-center justify-between pb-6 border-b border-slate-800 mb-6">
              ...
              <button onClick={() => setMobileMenuOpen(false)} className="p-1 hover:bg-slate-800 rounded-lg text-slate-400 hover:text-white transition">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            {/* Footer */}
            <div className="pt-6 border-t border-slate-800 space-y-3 mt-auto">
              <div className="flex items-center gap-3">
                {currentUser.avatar_url ? (
                  <img src={currentUser.avatar_url} alt="Profile" className="w-8 h-8 rounded-full object-cover border border-slate-700 bg-slate-800" />
                ) : (
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold truncate text-slate-200">{currentUser.full_name}</p>
                  <span className="inline-flex items-center px-1 py-0.5 rounded-full text-[8px] font-semibold bg-primary/15 text-primary border border-primary/20 mt-0.5">
                    {currentUser.role}
                  </span>
                </div>
              </div>
              <button
                onClick={onLogout}
                className="w-full flex items-center justify-center gap-1 py-2 bg-rose-950/20 hover:bg-rose-950/40 text-rose-300 rounded-lg text-xs font-semibold transition border border-rose-900/30"
              >
                <LogOut className="w-3.5 h-3.5" />
                <span>Logout</span>
              </button>
        ...
        <header className="hidden md:flex bg-white h-16 border-b border-slate-200 px-8 items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-2 text-slate-500 text-sm">
            <button 
              ...
              className="p-2 hover:bg-slate-100 rounded-xl transition border text-slate-500 mr-2 flex items-center justify-center"
